...
import base64
import os
import requests  # <--- 關鍵：用來傳送資料給 Google Apps Script
...
# --- 3. CSS 視覺樣式 (保留剛剛修好的手機版/深色模式/聚光燈) ---
st.markdown("""
    <style>
    /* 全站設定 */
    ...
    /* 強制文字深咖啡色 */
    h1, h2, h3, h4, h5, h6, .stMarkdown, p, div { color: #4E342E !important; }
    ...
    .stTabs [aria-selected="true"] { background-color: #FF9800 !important; color: white !important; }
    ...
    .card-title { 
        ...
        margin-bottom: 15px; letter-spacing: 2px; color: #000000 !important; 
    }
    .spotlight-box {
        background: radial-gradient(circle, #FFFFFF 30%, #BDBDBD 100%);
        padding: 20px; border-radius: 20px; text-align: center; margin-bottom: 20px;
        border: 1px solid #B0BEC5;
        box-shadow: inset 0 0 15px rgba(0,0,0,0.1);
    }
    .product-img { width: 100%; max-width: 280px; border-radius: 10px; filter: drop-shadow(0px 10px 10px rgba(0,0,0,0.3)); }
...
with tab1:
    ...
    st.markdown("""<div class="mom-box"><h4 style="color: #5D4037; text-align: center; font-size: 26px; margin-bottom: 10px;">從替家人把關，到大家的口耳相傳</h4><hr style="border: 0.5px solid #E0E0E0;"><p class="story-text">起初，這只是一份媽媽對家人的私房愛。<br><br>因為覺得外面的糖果價格不斐，成分又標示不清，熱愛料理的媽媽心想：<b>「既然家人愛吃，那就自己動手做吧！用最好的料，吃得才安心。」</b><br><br>當第一鍋「黑芝麻糖」出爐，那股濃郁的香氣立刻征服了左鄰右舍。大家一吃成主顧，紛紛驚呼：「這比外面的還好吃！」在親友的熱情推坑下，<b>【柴寶手作】</b>就這樣在大家的期待與祝福中，溫馨誕生了。</p></div>""", unsafe_allow_html=True)
    st.markdown("### 🐶 巷弄小太陽：福祿 (Lulu)")
    lc1, lc2 = st.columns(2)
    with lc1:
        try: st.image("lulu_q.png", use_container_width=True)
        except: st.info("缺少 lulu_q.png")
    with lc2:
        try: st.image("lulu_real.png", use_container_width=True)
        except: st.info("缺少 lulu_real.png")
    st.markdown("""<div class="story-box"><h4 style="color: #E65100; margin-bottom: 10px;">☀️ 溫暖的天使柴</h4><p class="story-text">鄰居們常說：<b>「看到祿祿，心情就好了一半。」</b><br>他是大家公認的「天使柴」。每當有人經過，他總是瞇著眼、開著飛機耳，用全身力氣搖著尾巴迎接。那種純真無邪的笑容，有一種讓人<b>瞬間忘記煩惱的魔力</b>。</p></div>""", unsafe_allow_html=True)
    st.markdown("### 🐦 用愛重生：喜寶 (Bobo)")
    bc1, bc2 = st.columns(2)
    with bc1:
        try: st.image("bobo_q.png", use_container_width=True)
        except: st.info("缺少 bobo_q.png")
    with bc2:
        try: st.image("bobo_real.png", use_container_width=True)
        except: st.info("缺少 bobo_real.png")
    st.markdown("""<div class="story-box" style="border-left: 8px solid #8D6E63;"><h4 style="color: #6D4C41; margin-bottom: 10px;">🐦 用愛灌溉的奇蹟</h4><p class="story-text">喜寶是媽媽從鳥販手中救下的孩子。當時牠受了傷，瑟瑟發抖。媽媽心疼收編後，這個孤僻的小傢伙把所有的溫柔都留給了媽媽。<br>廚房忙時，牠會撒嬌叫<b>「咪～」</b>；聽到媽媽回家，牠總是第一個大叫迎接。<b>牠的叫聲，是我們家最安心的信號。</b></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="five-elements"><h4 style="color: #E65100; font-size: 24px;">✨ 五行相生．財寶滾滾</h4><p class="story-text">黑柴屬水，水生木（柴），木生火（手作），火生土（財庫），土生金（財寶）。<br>福祿的笑容帶來了「人氣」，喜寶的重生帶來了「福氣」。<br>這就是<b>【柴寶手作】</b>的初心——我們賣的不只是甜點，更是一份被愛包圍的幸福好運。</p></div>""", unsafe_allow_html=True)

# ==========================================
# 分頁 2：美味下單 (使用 GAS 網址傳輸)
# ==========================================
with tab2:
    st.markdown("### ✨ 心靈祝禱系列")
        ...
        if submit:
            if not name or not phone:
                ...
                order_data = {
                    ...
                    "line_id": line_id,
                    "qty_sesame": q1,
                    "qty_cookie": qty_cookie_total, # 這裡把兩種口味加總傳給後端
